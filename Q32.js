...
let current_users;
current_users = ["Ahmed", "Ali", "Abbas", "Naqi", "Jawwad"];
let new_users;
new_users = ["AHmed", "Raza", "Abbas", "ALI", "Zaid"];
for (let new_user of new_users) {
    let isAvailable = true;
    for (let current_user of current_users) {
        if (new_user.toLowerCase() === current_user.toLowerCase()) {
            console.log(`Username "${new_user}" has already been used. Please choose a different username `);
            isAvailable = false;
            break;
        }
    }
